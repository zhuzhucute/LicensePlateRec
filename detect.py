...
def detect(weights='./weights/last.pt',
           source='./data/images/green',
           output='./data/outputs/green',
           img_size=640,
           conf_thres=0.4,
           iou_thres=0.5,
           device='cpu',
    ...
    for path, img, im0s, vid_cap in dataset:
        ...
        for i, det in enumerate(pred):  # detections per image
            ...
            save_path = str(Path(output)) + '/[Detect]'+ str(Path(p).name)
                    ...
                    *xyxy, conf, cls = de
                    ...
                    if save_img or view_img:  # Add bbox to image
                        ...
                        for a,i in enumerate(lic_plat):
                            ...
                            lb += CHARS[int(i)]
                        label = '%s %s' % (lb[:2], lb[2:])
                        im0=plot_one_box(xyxy, im0, label=label, color=colors, line_thickness=3)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--source', type=str, default='./data/images/green', help='source')  # file/folder, 0 for webcam
    parser.add_argument('--output', type=str, default='./data/outputs/green', help='output folder')  # output folder
    ...
    with torch.no_grad():
        if opt.update:  # update all models (to fix SourceChangeWarning)
            for opt.weights in ['yolov5s.pt', 'yolov5m.pt', 'yolov5l.pt', 'yolov5x.pt', 'yolov3-spp.pt']:
                detect()
                create_pretrained(weights, weights)
        else:
            detect()
